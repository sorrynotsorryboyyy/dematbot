...
import { divider, stack } from '../lib/layout.js';
...
const ARTICLES = [
  ...
  ['🛡️', '6 · Le staff', 'Les décisions du staff s appliquent. Un désaccord ? Ouvre un ticket, on en discute au calme.'],
];
...
export function buildMessage() {
  const embed = embeds.log({
    ...
    description: stack(
      ...
      divider(),
      '👇  **Accepte le règlement ci-dessous pour débloquer la communauté.**',
    ),
    fields: ARTICLES.map(([emoji, name, value]) => ({ name: `${emoji}  ${name}`, value })),
